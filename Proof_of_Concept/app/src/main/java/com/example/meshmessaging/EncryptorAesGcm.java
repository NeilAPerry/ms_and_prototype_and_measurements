...
import com.example.meshmessaging.CryptoUtils;
...
public class EncryptorAesGcm {
    ...
    public static byte[] encrypt(String msg, SecretKey key) throws Exception {

        // encrypt and decrypt need the same IV.
        // AES-GCM needs IV 96-bit (12 bytes)
        byte[] iv = CryptoUtils.getRandomNonce(IV_LENGTH_BYTE);

        byte[] encryptedText = EncryptorAesGcm.encryptWithPrefixIV(msg.getBytes(UTF_8), key, iv);

        return encryptedText;
    }

    public static String decrypt(byte[] ct, SecretKey key) throws Exception {

        String pt = EncryptorAesGcm.decryptWithPrefixIV(ct, key);
